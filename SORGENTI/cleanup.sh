...
# Sicuro: niente fuser, niente umount -f, solo path sotto /var/tmp/quelo-doctor-live-build

set -euo pipefail

LB_DIR="/var/tmp/quelo-doctor-live-build"
OLD_LB_DIR="$(cd "$(dirname "$0")/.." && pwd)/.build/live-build"
...
if [[ -d "${OLD_LB_DIR}" ]] || mount | grep -qF "${OLD_LB_DIR}/"; then
  echo "Vecchia build nel progetto..."
  quelo_cleanup_tree "${OLD_LB_DIR}" || exit 1
  rm -rf "$(dirname "${OLD_LB_DIR}")" 2>/dev/null || true
fi
